...
@Author: Jameson Thai
Visualizer Credit: Daniel Shiffman and source code comes from Rainbow Code
*/
import ddf.minim.*;
...
int circleSize;
boolean change = true;
color perlinColor; // Assigining color to the perlin noise
long startTime = System.nanoTime(); 
long endTime;
// need to create a data folder in perlin_Final, 
// then you can just call the mp3 file directly from there
String songName = "RunningInThe90s.mp3";
// Generating Random colors
float ColorRandomizer(){
 return random(360); 
}

// Generating circles at the end
class FadingCircle {
  int x, y;
  int size;
  color c;
  ...
  FadingCircle(int x, int y, int size) {
    ...
    this.x = x;
    this.y = y;
    // For choosing more appropriate circle colors based on mood https://processing.org/tutorials/color/
...
/**
* Initial background build for the field
**/
void setup()
{
  fullScreen(P3D);      
...
void draw() {
  flying -= 0.08; // need to reassign to the average tempo of the music
  float yoff = flying;
  for (int y = 0; y<rows; y++) {
    float xoff = 0;

    for (int x =0; x< cols; x++) {
      terrain[x][y] = map(noise(xoff, yoff ), 0, 1, -100, z); 
      xoff += 0.2;
    }
    yoff += 0.2;
  }
